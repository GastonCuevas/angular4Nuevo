    ...
    private findLeave(array: Array<any>, url: string, code: string): any {
        for (let d of array) {
            if (d.hasOwnProperty('leaves')) {
                const result = this.findLeave(d.leaves, url, code) || this.findLeave(d.children, url, code);
                if (!!result) return result;
                continue;
            }
            if (!!url && url.includes(d.webmnu.trim().toLowerCase()) || !!code && code === d.codigo) return d;
        }
        return null;
    }
}
