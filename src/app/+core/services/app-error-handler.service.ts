...
export class AppErrorHandlerService extends ErrorHandler {
    ...
    handleError(error: Error) {
        const copy_error = `Nombre: ${error.name} - Mensaje: ${error.message} - Stack: ${error.stack || 'sin stack'}  `;
        console.error(copy_error);
        if (this.router.url == '/camas/internaciones'
            ...
            this.toastyMessageService.showErrorMessagge('Reporte con errores: ' + error.message);
        }
    }
