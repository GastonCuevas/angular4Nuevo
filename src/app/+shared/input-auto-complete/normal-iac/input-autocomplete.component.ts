...
export class InputAutocomplete implements OnInit, OnChanges {
    ...
    @Input() source = new Array<ItemCombo>();
    ...
    @Output() valueChange = new EventEmitter<ItemCombo>();
    ...
    ngOnInit() { this.loadDataSource(); }

    ngOnChanges(changes: SimpleChanges) {
        if (this.readonlyIAC) return;
        ...
        console.log('ngOnChanges formControlNameIAC', this.formControlNameIAC, 'source', source, 'isLoading', isLoading);
    }

    private loadDataSource() {
        if (this.readonlyIAC) return;
        if (this.functionForData) this.loadDataByFunction();
        else this.loadDataBySource();
    }

    private loadDataByFunction() {
        this.isLoading = true;
    ...
    private loadDataBySource() {
        if (!this.source) {
            this.source = [];
            this.isLoading = true;
            this.notTouched = true;
        } // else if (this.source.length == 0) this.setStringList();
    }
    ...
    private updateDataSource(newDataSource: Array<ItemCombo>) {
    ...
    private replaceValues(value: string) {
        ...
        if (found) {
            ...
            if (formControlvalueIAC) this.setNewValueControl(new ItemCombo());
            else {
                this.valueAuxIAC = '';
                this.setClass();
            }
    ...
    private setNewValueControl(item: ItemCombo) {
    ...
    clearInput() {
        this.valueAuxIAC = '';
        this.setNewValueControl(new ItemCombo());
    }
